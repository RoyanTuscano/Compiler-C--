...
char *O1_optimization;
char *O2_optimization;

str_table *str_table_head;
symtabnode *SymTab_GL[2][256];
bool stnode_exist = false;
static int label_num = 0;
string_list *string_list_head;

symtabnode *temp_free_list[50];
symtabnode *temp_use_list[50];
array_elem_list array_used_list[50];
static int array_used_count = 0;
int temp_free_list_count = 0;
int temp_use_list_count = 0;
...
void enter_function(tnode *t, char *fun_name)
{
    ...
    temp_free_list_count = 0;
    temp_use_list_count = 0;
    sym_temp_ref = 0;
    string_list_head = NULL;
    stptr = SymTabLookup(fun_name, sc);
    array_used_count = 0;
    global_all_live = true;
    //  printf("%s , %s \n", O1_optimization, O2_optimization);
    if (print3code == 1)
    {
        //dump all globals
        printf("global\n");
    ...
    if (print3code == 1)
    {
        printf("4**********3 Address code*******\n");
        printSTNode(stptr);
        printf("********the instructions********\n");
        print_instruction(temp0->code);
    }
    if (strcmp(O1_optimization, "-O1") == 0)
    {
        //   printf("Doing Machine Independent optimization \n");
        machine_independent_optimization_jump(t);
        machine_independent_optimization_const(t);
        machine_independent_optimization_Bin_Unary(t);
        machine_independent_optimization_if_goto_con(t);
        begin_live_analysis(t);
        intra_block_cp_prop(t);
        find_gen_kill(t);
        find_in_out(t);
        identify_dead_code(t);
        //   identify_dead_code(t);
        remove_if(t);
        if (print3code == 1)
        {
            printf("3**********3 Address code*******\n");
            printSTNode(stptr);
            printf("********the instructions********\n");
            print_instruction(temp0->code);
        }
    ...
    //register allocation
    if (strcmp(O2_optimization, "-O2") == 0)
    {
        stack_print_once = false;
        register_allocation = 1;
        begin_live_analysis(t);
        // intra_block_cp_prop(t);
        find_gen_kill(t);
        find_in_out(t);
        reg_alloc_cal_live_range(t);
        start_graph_coloring(t);
    }
...
void code_gen_BOOL_EX(tnode *t, instr *L_then, instr *L_else)
{
    ...
    free_temp(stBinop_Op1(t)->place);
    t_instr->src2.val.stptr = stBinop_Op2(t)->place;
    free_temp(stBinop_Op2(t)->place);
    insert_code(&(t->code), t_instr);
    t_instr = newinstr(Goto_op, NONE, NONE, Intcon_operand);
...
void code_gen_array(tnode *t, int lr)
{
    ...
    insert_code(&(t->code), t_instr);
    free_temp(t_instr->src1.val.stptr);
    //plus
...
symtabnode *newtemp(int type)
{
    ...
    //check if there is free temp for the same type
    for (int i = 0; i < temp_free_list_count; i++)
    {
        if (temp_free_list[i]->type == type)
        {
            symtabnode *ntmp;
            ntmp = temp_free_list[i];
            temp_free_list[i] = NULL;
            temp_free_list[i] = temp_free_list[--temp_free_list_count];
            temp_use_list[temp_use_list_count++] = ntmp;
            return ntmp;
        }
    }
...
void code_gen_binary(tnode *t, Optype OP, int lr)
{
    ...
    else
    {
        ...
        codeGen_express(stBinop_Op1(t), R_value);
        codeGen_express(stBinop_Op2(t), R_value);
        t->place = newtemp(t->etype); //you can move this
        if (stBinop_Op1(t) != NULL)
            insert_code(&(t->code), stBinop_Op1(t)->code);
        if (stBinop_Op2(t) != NULL)
            insert_code(&(t->code), stBinop_Op2(t)->code);
        ...
        t_instr->src1.val.stptr = stBinop_Op1(t)->place;
        //check if the value is there in temp if there than free that temporary
        free_temp(stBinop_Op1(t)->place);
        t_instr->src2.val.stptr = stBinop_Op2(t)->place;
        free_temp(stBinop_Op2(t)->place);
        t_instr->dest.val.stptr = t->place;
...
void free_temp(symtabnode *str)
{
    if (str == NULL)
        return;
    for (int i = 0; i < temp_use_list_count; i++)
    {
        if (temp_use_list[i] == str)
        {
            temp_free_list[temp_free_list_count++] = str;
            temp_use_list[i] = NULL;
            temp_use_list[i] = temp_use_list[--temp_use_list_count];
            return;
        }
    }
...
void Charcon_code_assign(tnode *t)
{
    ...
    //free the temp
    // temp_free_list[temp_free_list_count++]=t->place;
}
...
{
    //you can elemenate this
    t->code = newinstr(Assn_op, Stringcon_operand, NONE, Stringcon_operand);
    t->code->src1.val.str_const = stStringcon(t);

    bool found = false;
    string_list *t_string = string_list_head;
    string_list *prev_string = NULL;
    symtabnode t_place;
    while (t_string != NULL)
    {
        if (strcmp(t_string->string_name, stStringcon(t)) == 0) //t_string->string_name == stStringcon(t))
        {
            t->place = t_string->t_stptr;
            found = true;
            break;
        }
        prev_string = t_string;
        t_string = t_string->next;
    }
    if (found == false)
    {
        ...
        if (prev_string != NULL)
        {
            t_string = prev_string;
            t_string->next = malloc(sizeof(string_list));
            t_string->next->next = NULL;
            t_string->next->string_name = stStringcon(t);
            t_string->next->t_stptr = t->place;
        }
        else
        {
            t_string = malloc(sizeof(string_list));
            t_string->next = NULL;
            string_list_head = t_string;
            t_string->string_name = stStringcon(t);
            t_string->t_stptr = t->place;
        }
    }
