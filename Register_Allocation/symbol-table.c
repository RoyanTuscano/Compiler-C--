...
int local_arg()
{
  ...
  for (i = 0; i < HASHTBLSZ; i++)
  {
    ...
    {
      //  printSTNode(stptr);
      if (stptr->type != t_Char && check_vertex_exist(stptr, &g_head) == true && register_allocation == 1 && stptr->formal == false && stptr->type != t_Array)
      {
        if (stack_print_once == true)
          printf("#No stack node = %s \n", stptr->name);
      }
      else
      {
        if (stack_print_once == true)
          printf("#stack node = %s \n", stptr->name);
        //if (stptr->formals == false)
        j++;
      }
    }
  }
  ...
  for (int i = 0; i < HASHTBLSZ; i++)
  {
    for (stptr = SymTab[Local][i]; stptr != NULL; stptr = stptr->next)
    {
      if (stptr->formal == false)
      {
        ...
        if (stptr->type != t_Char && check_vertex_exist(stptr, &g_head) == true && register_allocation == 1 && stptr->type != t_Array)
        {
        }
        else
        {
